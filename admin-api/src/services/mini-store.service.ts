  ...
  const products = resolveStoreProducts(item)
    ...
    .map((entry) => {
      ...
      return {
        ...
        detailItems: Array.isArray(entry.detailItems) ? entry.detailItems : [],
...
function mapAdminEditableProduct(item: any) {
  return {
    ...
    detailTitle: String(item.detailTitle || ""),
...
export async function getAdminStoreDashboard(adminUserId: number, storeId: number, rawQuery: Record<string, unknown> = {}) {
  ...
  const productList = products.map((product: any) => {
    ...
    return {
      ...
      detailTitle: String(product.detailTitle || ""),
      detailText: String(product.detailText || ""),
      detailItems: Array.isArray(product.detailItems) ? product.detailItems : [],
