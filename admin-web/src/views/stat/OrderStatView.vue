...
import type { OrderStatResult } from "../../api/modules/stat";
import { getOrderStatApi } from "../../api/modules/stat";

const loading = ref(false);
const data = ref<OrderStatResult>({ cards: [], schoolRank: [] });

async function loadData() {
  loading.value = true;
  try {
    data.value = await getOrderStatApi();
  } finally {
    loading.value = false;
  }
}
