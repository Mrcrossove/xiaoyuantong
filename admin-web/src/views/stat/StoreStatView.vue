...
import type { StoreStatResult } from "../../api/modules/stat";
import { getStoreStatApi } from "../../api/modules/stat";

const loading = ref(false);
const data = ref<StoreStatResult>({ cards: [], schoolRank: [] });

async function loadData() {
  loading.value = true;
  try {
    data.value = await getStoreStatApi();
  } finally {
    loading.value = false;
  }
}
